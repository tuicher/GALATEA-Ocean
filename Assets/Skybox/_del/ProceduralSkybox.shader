    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(a2v v)
            {
                ...
                o.milkyWayPos = mul((float3x3)_MilkyWayWorld2Obj, v.positionOS.xyz) * _MilkyWayTex_ST.xyz;
            ...
            half4 frag(v2f i): SV_Target
            {
                ...
                float star = SAMPLE_TEXTURE2D(_StarTex, sampler_StarTex, sphereUV  * 2).r;
                star = saturate(star * star * star * 3) * _StarIntensity;

                //return float4(saturate(i.moonPos.xy), 0, 1);
                //half4 moon = SAMPLE_TEXTURE2D(_MoonTex, sampler_MoonTex, (i.moonPos.xy % 1.0 + 0.5)) * step(0.5, dot(normalizePosWS, -_MoonDirectionWS.xyz));
